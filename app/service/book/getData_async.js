...
const getUpdateList = (book) => {
	return new Promise(async (resolve, reject) => {
        ...
        // TODO
        // res.splice(2, res.length-1);

        const promiseArr = res.map((item, index) => {
            return new Promise(async (resolve, reject) => {
                // 接口防刷，并发可能会被拦截，若被拦截则2s后重试。重试机制最多5次。失败则按status:defect处理，待后续补全缺损数据流程处理。
                let content = false;
                let n = 0;
                console.log(`开始读取《${book.name}》章节数据(${index})`);
                while(!content && n<5) {
                    content = await getDetail(item.href);
                    n++;
                }
                content = content||'defect';
                console.log(`读取完成《${book.name}》章节数据(${index})`);
                resolve(content);
            });
        });
        // 此处并发请求规则对于新书可能不是很友好,后续优化下
        Promise.all(promiseArr).then((result) => {
            for(let i=0; i<res.length; i++) {
                res[i].content = result[i]||'';
	...
	return new Promise((resolve, reject) => {
        const promiseArr = list.map((item) => {
            return new Promise(async (resolve, reject) => {
                ...
                let n = 0;
                while(!content && n<5) {
                    console.log(`开始读取第${n+1}本书数据`);
                    content = await getUpdateList(item);
                    console.log(`读取完成第${n+1}本书数据`);
                    n++;
                }
                // content = content||'defect';
                resolve(content);
